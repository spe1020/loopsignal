/** Company-level brand strings. Domain stays env-driven via `lib/site.ts`. */

export const company = {
  name: "LoopSignal",
  contactEmail: "hello@loopsignal.co",
  location: "Central Pennsylvania",
  // Unpublished until a number is confirmed. Do not invent one.
  phone: "",
  tagline: "Improve the process. Connect the systems.",
  supportingTagline: "Find the signal. Close the loop.",
  description:
    "LoopSignal helps manufacturers improve processes, connect disconnected systems, and automate the work that should not require manual effort.",
  longDescription:
    "LoopSignal is a manufacturing consulting and systems integration company. We start with how the work actually happens, then improve the process and connect the right systems, information, and technology around it.",
  executiveDescription:
    "LoopSignal builds manufacturing problem-solving software, with optional onboarding and integration support. Explore a fictional browser-local product preview today.",
  philosophy: "Start with the work.",
  operatingPhilosophy:
    "Technology follows the problem, not the other way around.",
} as const;
